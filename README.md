...
#### 2. 做题策略
1. 没有什么基础知识的情况下可以**每个专题都先只做 1-3 道简单或中等题了解主要思想和内容**，“结构”内的专题可以多做几题。若有一定基础可自行决定跳过部分专题或减少题量。该阶段各专题的推荐入门题目见[清单](List.md)内加粗的题号。
    > 目的是了解各数据结构的基本实现和各算法的概念和原理。

2. 每个专题都了解了后再按上面的顺序**从头刷每个专题，每个专题约 10-20 题**，数量可根据专题难度和重要程度调整。
    > 目的是掌握各数据结构实现和操作的基本模板、各算法的基本模板、提高编码熟练度。
