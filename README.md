### 一、知识点
下面的专题除个别以外都能在 leetcode 上找到对应的习题集，专题后面的题号对应了我相应专题的 leetcode 刷题顺序，可供参考。

#### 1. 结构
* **哈希表**：1, 13, 36, 242, 454
* **数组**：
* **链表**：
* **字符串**：
* **栈**：
	* 栈：
	* 单调栈：
* **队列**：
	* 队列：
	* 单调队列：
* **树**：
	* 树：
	* 二叉树：226, 101, 104, 111, 222, 257, 404, 513, 112, 106, 654, 617, 
	* 二叉搜索树(Binary Search Tree, BST, 二叉查找树或二叉排序树)：
	* 平衡二叉树
    	* 平衡二叉树：110
    	* 红黑树
	* 树状数组(二叉索引树)：
	* 堆(优先队列)：
	* 线段树：
	* 字典树：
	* 并查集(集合)：
	* B树家族(主要用于磁盘、数据库设计，而非内存算法)
* **有序集合**：
* **图**：997, 1971

#### 2. 算法
* **组别一：**  
  * 查找：
  * 排序：
* **组别二：**
  * 回溯：17, 22, 51, 
  * 递归：
  * 分治：
* **组别三：**
  * 双指针：202
  * 滑动窗口：3,
* **组别四：**
  * 广度优先搜索(BFS)：
  * 深度优先搜索(DFS)：
* **组别五：**
  * 贪心：455,
  * 动态规划：5, 

---

### 二、leetcode做题顺序
数据结构与算法显然分“结构”与“算法”两个部分，它们结合紧密的同时，又有依赖关系，一个算法往往是针对某种结构设计的，结构的选择影响了算法的选择。因此采取了先刷结构专题后刷算法专题的策略。

> 结构->组别一->组别二->组别三->组别四->组别五

1. 没有什么基础的情况下可以每个专题都先只做 1-3 道简单或中等题了解主要思想和内容，“结构”内的专题可以多做几题，后面常用
2. 每个专题都了解了后再从头刷每个专题，每个专题约 10-20 题
3. 对每个专题有一定刷题量后，可以深入个别专题学习或混合练习
4. 难点:
   * 回溯
   * 递归
   * 分治
   * 贪心
   * 动态规划

---

### 三、学习资源
1. 参考书籍
* 入门：《大话数据结构》
* 进阶：《算法导论》
* 拓展：《算法竞赛入门经典》​ (刘汝佳，紫书)

2. 开源社区
* [hello-algo](https://github.com/krahets/hello-algo)
* [fucking-algorithm](https://github.com/labuladong/fucking-algorithm)
* [OI Wiki​](https://github.com/OI-wiki/OI-wiki)
  * 网址: oi-wiki.org
  * [状态页](https://status.oi-wiki.org): 用于监控 OI Wiki 站点的在线情况。如果遇到了无法访问的问题，可以打开状态页，寻找可以连接的镜像站

3. 数学社区
* [Mathematics Stack Exchange​](math.stackexchange.com)
* [MathOverflow​​](mathoverflow.net)
* [Art of Problem Solving (AoPS)​​](artofproblemsolving.com)

4. 其它题库
* [洛谷](https://www.luogu.com.cn/)